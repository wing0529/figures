...
REPO_ROOT = FIGURE_ROOT.parent

DEFAULT_JSONL_PATHS = [
    REPO_ROOT / "logs" / "ppl_runs_harness.jsonl",
    REPO_ROOT / "logs" / "ppl_runs_harness copy.jsonl",
    REPO_ROOT / "logs" / "acc_runs_harness.jsonl",
    REPO_ROOT / "logs" / "acc_runs_harness_backup260511.jsonl",
]
    ...
    while idx < length:
        ...
        try:
            ...
            if next_obj >= 0:
                idx = next_obj
                continue
            raise ValueError(f"{path}:{lineno} is not valid JSONL. CSV inputs are auto-detected by .csv suffix.") from exc
